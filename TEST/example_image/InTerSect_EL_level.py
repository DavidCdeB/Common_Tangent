...
filefolder_Calcite_I_SG_167 = '../Files_Outputs/Calcite_I'

filefolder_Calcite_II_SG_14 = '../Files_Outputs/Calcite_II'
...
V_C_I_lin = np.linspace(V_C_I[0]-1.5, V_C_I[-1], 10000)
V_14_lin = np.linspace(V_14[0], V_14[-1]+1, 10000)
...
plt.legend((p1, p2, p5, p6, pcomm_tangent), ("Calcite I", "$f_{1}$: Cubic fit Calcite I", "Calcite II", '$f_{2}$: Cubic fit Calcite II', 'Common tangent'), prop=fontP)

print 'devBM(x1, popt_C_I[1], popt_C_I[2], popt_C_I[3]) = ', devBM(x1, popt_C_I[1], popt_C_I[2], popt_C_I[3])

y1 = BM(x1, popt_C_I[0], popt_C_I[1], popt_C_I[2], popt_C_I[3])
y2 = BM(x2, popt_14[0], popt_14[1], popt_14[2], popt_14[3])
print 'y1 = ', y1
#sys.exit()
# vertical line
plt.vlines(x=x1, ymin=-941.258, ymax=y1, color='orange', linestyle='--', zorder=2)
plt.vlines(x=x2, ymin=-941.258, ymax=y2, color='orange',  linestyle='--', zorder=2)
plt.text(60.1, -941.2578, '$x_{1}$', color='orange')#,rotation=90)
plt.text(61.5, -941.2578, '$x_{2}$', color='orange')#,rotation=90)

plt.hlines(y=y1, xmin=53.2957, xmax=x1, color='orange', linestyle='--', zorder=2)
plt.hlines(y=y2, xmin=53.2957, xmax=x2, color='orange',  linestyle='--', zorder=2)
plt.text(53.4, -941.2537, '$f(x_{1})$', color='orange')#,rotation=90)
plt.text(53.4, -941.255, '$f(x_{2})$', color='orange')#,rotation=90)


#plt.axvline(x=x1, ymin=0, ymax=y1, color='red', zorder=2)
...
plt.gca().set_ylim(bottom=-941.258)#, top=350)
plt.gca().set_xlim(left=53.2957)#, top=350)
